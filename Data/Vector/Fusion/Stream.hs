...
-- | Singleton 'Stream'
singleton :: a -> Stream v a
{-# INLINE singleton #-}
singleton = M.singleton

-- | Replicate a value to a given length
replicate :: Int -> a -> Stream v a
{-# INLINE replicate #-}
replicate = M.replicate

-- | Generate a stream from its indices
generate :: Int -> (Int -> a) -> Stream v a
{-# INLINE generate #-}
generate = M.generate
...
elem :: Eq a => a -> Stream v a -> Bool
{-# INLINE elem #-}
elem x = unId . M.elem x

infix 4 `notElem`
-- | Inverse of `elem`
notElem :: Eq a => a -> Stream v a -> Bool
{-# INLINE notElem #-}
notElem x = unId . M.notElem x

-- | Yield 'Just' the first element matching the predicate or 'Nothing' if no
-- such element exists.
find :: (a -> Bool) -> Stream v a -> Maybe a
{-# INLINE find #-}
find f = unId . M.find f

-- | Yield 'Just' the index of the first element matching the predicate or
-- 'Nothing' if no such element exists.
findIndex :: (a -> Bool) -> Stream v a -> Maybe Int
{-# INLINE findIndex #-}
...
and :: Stream v Bool -> Bool
{-# INLINE and #-}
and = unId . M.and

or :: Stream v Bool -> Bool
{-# INLINE or #-}
or = unId . M.or
...
-- | Unfold
unfoldr :: (s -> Maybe (a, s)) -> s -> Stream v a
{-# INLINE unfoldr #-}
unfoldr = M.unfoldr

-- | Unfold at most @n@ elements
unfoldrN :: Int -> (s -> Maybe (a, s)) -> s -> Stream v a
{-# INLINE unfoldrN #-}
unfoldrN = M.unfoldrN

-- | Apply function n-1 times to value. Zeroth element is original value.
iterateN :: Int -> (a -> a) -> a -> Stream v a
{-# INLINE iterateN #-}
iterateN = M.iterateN
...
instance Ord a => Ord (M.Stream Id v a) where
  {-# INLINE compare #-}
  compare = cmp
...
mapM_ :: Monad m => (a -> m b) -> Stream v a -> m ()
{-# INLINE mapM_ #-}
...
-- | Monadic fold
foldM :: Monad m => (a -> b -> m a) -> a -> Stream v b -> m a
{-# INLINE foldM #-}
foldM m z = M.foldM m z . liftStream

-- | Monadic fold over non-empty stream
fold1M :: Monad m => (a -> a -> m a) -> Stream v a -> m a
{-# INLINE fold1M #-}
...
-- @x+y+y@ etc.
enumFromStepN :: Num a => a -> a -> Int -> Stream v a
...
enumFromTo :: Enum a => a -> a -> Stream v a
{-# INLINE enumFromTo #-}
...
-- 'enumFromStepN' instead.
enumFromThenTo :: Enum a => a -> a -> a -> Stream v a
...
fromListN :: Int -> [a] -> Stream v a
{-# INLINE fromListN #-}
fromListN = M.fromListN

unsafeFromList :: Size -> [a] -> Stream v a
